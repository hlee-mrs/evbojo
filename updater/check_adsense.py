#!/usr/bin/env python3
"""[수동 진단 전용] 애드센스 광고 실게재 감지 — 라이브 사이트를 헤드리스로 열어 광고 iframe을 확인.

자동 파이프라인(run_adsense_watch.sh)에서는 더 이상 호출하지 않는다(API 단독 전환).
광고가 실제로 나가는지 눈으로 확인하고 싶을 때 터미널에서 직접 실행하는 용도.
